...
function mergeTxPhotoRows(existingTx, incomingTx) {
  ...
  // 聯集合併：不可只保留 incoming，否則空陣列 sync 會把雲端帳本整個洗掉
...
function mergeGameState(existing, incoming) {
  ...
  if (Array.isArray(incoming.tx)) {
    const clientAckWipe = incomingWipe >= existingWipe;
    const existingTx = Array.isArray(existing.tx) ? existing.tx : [];
    const incomingEmpty = incoming.tx.length === 0;
    const existingHas = existingTx.length > 0;

    // 雲端已洗過，但客戶端還帶舊交易、尚未承認 wipe → 拒絕回灌
    if (existingWipe > 0 && !clientAckWipe) {
      merged.tx = existingTx;
    } else if (existingWipe > 0 && existingTx.length === 0) {
      // 洗空後只接受「承認 wipe」後的客戶端交易（通常也是空的或全新記帳）
      merged.tx = clientAckWipe ? incoming.tx : [];
    } else if (incomingEmpty && existingHas && incomingWipe <= existingWipe) {
      // 防護：沒有更新的 wipe 時，空 tx 不得覆寫雲端既有帳本／相簿
      merged.tx = existingTx;
    } else {
      merged.tx = mergeTxPhotoRows(existingTx, incoming.tx);
    }
  }
