...
function mergeGameState(existing, incoming) {
  ...
  const wipedAt = Math.max(+(existing._historyWipedAt || 0), +(incoming._historyWipedAt || 0));
  if (wipedAt > 0) merged._historyWipedAt = wipedAt;

  if (Array.isArray(incoming.tx)) {
    const clientAckWipe = +(incoming._historyWipedAt || 0) >= +(existing._historyWipedAt || 0);
    // 雲端已洗過，但客戶端還帶舊交易、尚未承認 wipe → 拒絕回灌
    if (+(existing._historyWipedAt || 0) > 0 && !clientAckWipe) {
      merged.tx = Array.isArray(existing.tx) ? existing.tx : [];
    } else if (+(existing._historyWipedAt || 0) > 0 && Array.isArray(existing.tx) && existing.tx.length === 0) {
